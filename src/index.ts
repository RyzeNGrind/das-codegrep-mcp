 ...
 * ─────────────────────────────────────────
 * Transport : stdio (NixOS-WSL safe, no port conflict)
 * Search    : Zoekt trigram (Sourcegraph-grade, 100% offline)
 * Guard     : pre-ingress bad-pattern scanner
 *
 * Tools:
 *   search_code      — Zoekt trigram search (lang:nix, f:*.nix, regex)
 *   index_directory  — build/update local Zoekt index
 *   guard_code       — scan snippet before writing to workspace
 *   guard_file       — scan existing file
 *   search_file      — filename/glob search in workspace
 *   read_file        — read local file (line-range aware)
...
const GUARD: GuardPattern[] = [
  { id:"hardcoded-secret",  label:"Hardcoded secret/token",          severity:"error",
    regex:/(api[_-]?key|secret|token|password)\s*=\s*["'][^"']{8,}["']/i,
    tip:"Use sops-nix, age, or environment variables." },
  { id:"eval-usage",        label:"eval() call",                     severity:"error",
    regex:/\beval\s*\(/,
    tip:"eval() is a code-injection vector. Refactor." },
  { id:"rm-rf",             label:"rm -rf without guard",            severity:"error",
    regex:/rm\s+-rf?\s+[^$\{]/,
    tip:"Add path validation or use safer deletion patterns." },
  { id:"curl-pipe-sh",      label:"curl|sh / wget|bash",             severity:"error",
    regex:/curl[^|]+\|\s*(ba)?sh|wget[^|]+\|\s*(ba)?sh/,
    tip:"Verify checksums — never pipe curl/wget to shell." },
  { id:"console-log",       label:"console.log in source",           severity:"warn",
    regex:/console\.log\(/,
    tip:"Use a structured logger for production code." },
  { id:"todo-fixme",        label:"TODO/FIXME left in code",          severity:"warn",
    regex:/\b(TODO|FIXME|HACK|XXX)\b/,
    tip:"Track in your issue tracker instead." },
  { id:"empty-catch",       label:"Empty catch block",               severity:"warn",
    regex:/catch\s*\([^)]*\)\s*\{\s*\}/,
    tip:"Always handle or re-throw errors." },
  { id:"any-type",          label:"TypeScript 'any' type",           severity:"warn",
    regex:/:\s*any\b/,
    tip:"Use specific types or 'unknown' + type narrowing." },
  { id:"nix-fetchurl-nohash", label:"fetchurl/fetchTarball without hash", severity:"error",
    regex:/fetch(url|Tarball)\s*\{[^}]*url[^}]*\}/,
    tip:"Pin fetchurl/fetchTarball with sha256 for reproducibility." },
  { id:"nix-with-pkgs",     label:"with pkgs; anti-pattern",         severity:"warn",
    regex:/with\s+pkgs\s*;/,
...
async function zoektSearch(query:string, max=15): Promise<ZoektResult[]> {
  try {
    ...
    if (e?.code==="ECONNREFUSED")
      throw new Error(`Zoekt not running on ${ZOEKT_PORT}. Run: ./bin/dev-up`);
    throw e;
  }
}
  ...
  try {
    switch (name) {
        ...
        const out = rs.map(r => {
          const snips = r.lines.map(l =>
            [...l.before.map((b,i)=>`  ${l.lineNumber-l.before.length+i} │ ${b}`),
             `→ ${l.lineNumber} │ ${l.line}`,
             ...l.after.map((a,i)=>`  ${l.lineNumber+1+i} │ ${a}`)].join("\n")
          ).join("\n---\n");
      ...
      case "index_directory": {
        ...
        return {content:[{type:"text",text:`✅ Indexed ${directories.length} dir(s) → \`${INDEX_DIR}\`\n${stderr||""}`}]};
      }

      case "guard_code": {
        const {code, filename="snippet"} = args as {code:string;filename?:string};
        const fs2 = guardCode(code, filename);
        if (!fs2.length) return {content:[{type:"text",text:`✅ Guard PASS: \`${filename}\` — no issues.`}]};
        const errors = fs2.filter(f=>f.severity==="error");
        const warns  = fs2.filter(f=>f.severity==="warn");
        const fmt = (f:GuardFinding) =>
          `**[${f.severity.toUpperCase()}]** \`${f.label}\` — Line ${f.line}\n> \`${f.lineText}\`\n> 💡 ${f.tip}`;
        return {content:[{type:"text",text:[
          `# 🛡 Guard: \`${filename}\``,
          errors.length ? `## ❌ Errors\n${errors.map(fmt).join("\n\n")}` : "",
          warns.length  ? `## ⚠️ Warnings\n${warns.map(fmt).join("\n\n")}` : "",
          `\n${errors.length} error(s), ${warns.length} warning(s)${
            errors.length>0 ? " — 🚫 fix before committing." : ""}`,
        ].filter(Boolean).join("\n\n")}]};
      }
      ...
      case "guard_file": {
        ...
        const fs2  = guardCode(code, path.basename(filePath));
        if (!fs2.length) return {content:[{type:"text",text:`✅ Guard PASS: \`${filePath}\``}]};
        const fmt = (f:GuardFinding) =>
          `- **[${f.severity.toUpperCase()}]** L${f.line}: \`${f.label}\` — ${f.lineText}\n  💡 ${f.tip}`;
      ...
      case "list_index": {
        ...
        return {content:[{type:"text",text:`## Indexed repos (${repos.length})\n${repos.map(r=>`- \`${r}\``).join("\n")||"None."}`}]};
      }

      case "zoekt_status": {
        try {
          await axios.get(`${ZOEKT_BASE}/`,{timeout:2000});
          return {content:[{type:"text",text:`✅ Zoekt running @ \`${ZOEKT_BASE}\`\nIndex: \`${INDEX_DIR}\``}]};
        } catch {
          return {content:[{type:"text",text:`❌ Zoekt NOT running.\nStart: ./bin/dev-up`}]};
        }
      }
